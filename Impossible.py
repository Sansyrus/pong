...
class Impossible:
    def __init__(self, w, h, game):
        ...
        self.scoreJ = 0
        self.scoreIA = 0
        self.ball = Pong((w//2), (h//2), self.game)
    ...
    def run(self, events):
        ...
        self.paddle_ia.follow_ball(self.ball.rect.centery, speed=7, screen_height=self.h)

        #collision balle paddleJ
        if self.ball.rect.colliderect(self.paddle.rect):
            self.ball.bounce_off_paddle(-1, self.paddle)
            self.ball.rect.left = self.paddle.rect.right

        #collision balle paddleiIA
        if self.ball.rect.colliderect(self.paddle_ia.rect):
            self.ball.bounce_off_paddle(1, self.paddle_ia)
