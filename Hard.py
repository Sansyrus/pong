...
class Hard:
    ...
    def run(self, events):
        ...
        self.paddle_ia.follow_ball(self.ball.rect.centery, speed=10, screen_height=self.h)


        #collision balle paddleJ
        if self.ball.rect.colliderect(self.paddle.rect):
            self.ball.bounce_off_paddle(-3, self.paddle)
            self.ball.rect.left = self.paddle.rect.right

        #collision balle paddleiIA
        if self.ball.rect.colliderect(self.paddle_ia.rect):
            self.ball.bounce_off_paddle(3, self.paddle_ia)
